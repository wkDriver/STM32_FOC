...
// 测试参数输出	
     TestPare.fact_BUS_Voil= ADCSampPare.BUS_Voltage;
	   ...
	   pi_spd.Fbk = EQEPPare.Speed_RPM  ;  //   0---4096RPM 
	  ...
	  AngleSin_Cos.IQAngle= EQEPPare.JZElecTheta ; // 
    ...
    pi_id.Ref = 0;
	  ...
	  pi_iq.OutF= _IQ10mpy(FilK1,pi_iq.OutF)+_IQ10mpy(FilK2,pi_iq.Out);
	   ...
	   {
	    IparkU.Ds=0;   //  采用开环
	    IparkU.Qs= 25*pi_spd.Ref;  //根据频率速度给电压 VF固定比例
	   }

	   if(logicContr.Run_mode==2)
	   {
	  	IparkU.Ds= pi_id.OutF;   // 采用id电流闭环 id电流闭环pi_id.OutF
	  	IparkU.Qs= pi_spd.OutF;  // 采用速度闭环  
	   }
     if(logicContr.Run_mode==3)
	   {
       IparkU.Ds= -pi_id.OutF;   // 采用速度闭环  id电流闭环
       IparkU.Qs= -pi_spd.OutF;  // 反转控制
     }
   ...
   TIM_SetCompare3(TIM4 , (EQEPPare.JZElecTheta>>5)); // DAC通道1   
	 TIM_SetCompare4(TIM4 ,  EQEPPare.Speed_RPM ); // DAC通道2 
} 
